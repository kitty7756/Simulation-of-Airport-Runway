...
class Queue
{
 public:
  ...
  void move_to_front(int);
  /*-----------------------------------------------------------------------
    Remove value with value int, move to the front.

    Precondition:  Queue is nonempty.
    Postcondition: search through a queue for a particular key value, and 
        if found, move the node to the front of the same queue.
  -----------------------------------------------------------------------*/

  void merge_two_queues(Queue & anotherQueue);
